...
| `minimatch@<4` | `^3.1.4` | Version-range selector (pnpm-specific): only applies to minimatch v3.x instances. Transitive via `eslint@9.35.0` (through `@eslint/config-array`, `@eslint/eslintrc`) and `eslint-plugin-import`, `eslint-plugin-node`, `eslint-plugin-react`. Lockfile froze at `3.1.2` (vulnerable to ReDoS). Selector isolates fix to v3 instances without affecting v9 instances. | Run `pnpm why minimatch`; remove when all parents resolve to >=3.1.4 naturally. Currently blocked by eslint's transitive resolution. | CVE-2026-26996, CVE-2026-27903, CVE-2026-27904 |
| `minimatch@>=9 <10` | `^9.0.7` | Version-range selector for minimatch v9.x instances. Transitive via `jest@30.1.3` (through `test-exclude`, `glob`), `np@10.2.0` (through `ignore-walk`), and `@typescript-eslint/eslint-plugin` (through `typescript-estree`). Lockfile froze at `9.0.5` (vulnerable to ReDoS). | Run `pnpm why minimatch`; remove when all parents resolve to >=9.0.7 naturally. | CVE-2026-26996, CVE-2026-27903, CVE-2026-27904 |
| `flatted` | `^3.4.2` | Transitive via `eslint@9.35.0` -> `file-entry-cache@8.0.0` -> `flat-cache@4.0.1` -> `flatted@3.3.3`. No parent upgrade resolves this (crawl returned null for eslint minimum fix version). Override forces safe version. | Run `pnpm why flatted`; remove when eslint upgrades its `file-entry-cache`/`flat-cache` chain to pull flatted >=3.4.2 naturally. | CVE-2026-32141, CVE-2026-33228 |
| `picomatch@<3` | `^2.3.2` | Version-range selector for picomatch v2.x instances. Transitive via `lint-staged@15.5.2`, `ts-loader@9.5.4`, `copy-webpack-plugin@11.0.0`, and `np@10.2.0` (all through `micromatch@4.0.8` -> `picomatch@2.3.1`). Lockfile froze at `2.3.1` (vulnerable to ReDoS). | Run `pnpm why picomatch`; remove when `micromatch` releases a version pulling picomatch >=2.3.2 and parent lockfile entries re-resolve. | CVE-2026-33671 |
| `picomatch@>=4 <5` | `^4.0.4` | Version-range selector for picomatch v4.x instances. Transitive via `jest@30.1.3` and `ts-jest@29.4.9` (through `jest-util@30.0.5`). Lockfile froze at `4.0.3` (vulnerable to ReDoS). | Run `pnpm why picomatch`; remove when jest-util releases a version pulling picomatch >=4.0.4 naturally. | CVE-2026-33671 |
| `fast-uri` | `^3.1.2` | Transitive via `ajv@8.20.0` -> `schema-utils` -> multiple webpack plugins (babel-loader, copy-webpack-plugin, mini-css-extract-plugin, terser-webpack-plugin, webpack). Lockfile froze at `3.1.0` (vulnerable to ReDoS). Single major (3.x only) so a global override is safe. | Run `3pp-util test-override-redundancy fast-uri`; remove when it exits 0 after ajv re-resolves fast-uri to >=3.1.2 naturally. Reviewer-local: `pnpm why fast-uri`. | GHSA-v39h-62p7-jpjc, GHSA-q3j6-qgpj-74h6 |
